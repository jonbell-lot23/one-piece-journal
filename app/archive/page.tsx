...
function GapCard({ gap, index }: { gap: GapEntry; index: number }) {
  ...
  return (
    ...
    >
      <div className="text-xs text-neutral-500 mb-2">{daysDiff} days later</div>
      <div className="text-sm text-neutral-600 italic">{gap.description}</div>
    </div>
  );
}
      ...
      .then((data) => {
        ...
        for (let i = 0; i < sorted.length; i++) {
          ...
          if (i < sorted.length - 1) {
            ...
            if (daysDiff > 30) {
              // Only show gaps longer than 30 days
              let gapDescription = "";
              if (daysDiff > 365) {
